...
# 🛰️ TP2 — Simulation d'une mission spatiale avec Xenomai (MI11 - 2025)

## 📌 Contexte pédagogique

Ce TP, réalisé dans le cadre du module MI11 à l'Université de Technologie de Compiègne (UTC), vise à concevoir une simulation temps réel de type "Pathfinder" à l’aide de **Xenomai 3**, sur un noyau Linux RT patché, avec l’API **Alchemy**.

L’objectif est de simuler une mission embarquée temps réel avec plusieurs tâches périodiques, des ressources critiques, des synchronisations, et une surveillance de comportement.

---

## 🚀 Objectifs

* Mettre en œuvre des tâches **périodiques temps réel** sous Xenomai
* Synchroniser les tâches avec un point de départ commun (`first_release_point`, `start_sem`)
* Simuler du **temps CPU consommé** avec `busy_wait()`
* Gérer des **ressources critiques** et des zones protégées
* Ajouter une tâche de **surveillance** pour le redémarrage automatique
...
Chaque tâche est représentée par une structure `task_descriptor` :
...
```c
RT_SEM start_sem;  // Toutes les tâches attendent ce sémaphore pour démarrer en même temps
```

### Démarrage synchrone des tâches
...
## 📦 Fonction busy\_wait : simulation correcte du temps CPU consommé
...
* Simulation **réaliste** d’un traitement CPU
* Ne dépend **pas des préemptions ou interruptions**
* Permet de reproduire fidèlement une **charge CPU déterminée**
